...
const RunWorkflow: FC<RunWorkflowProps> = ({ isLoggedIn, showToast }) => {
  // Pdfjs worker
  ...
  const [workflow_attachments, set_workflow_attachments] = useState<any[]>([]);
  ...
  const [tags_error, set_tags_error] = useState<string[]>([]);
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();

  const [showModal, setShowModal] = useState(false)
  const [runId, setRunId] = useState<any>('')
  ...
  const handleChangeTag = (isKey: boolean, i: number, e: any) => {
    let tempTags = tags;
    if (isKey) {
  ...
  const handleRemoveTag = (i: number) => {
    ...
    tempTags = [...tempTags];
    setTags(tempTags);
    const tempTagErrors = tags_error.splice(i, 1);
    set_tags_error([...tempTagErrors]);
  };
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    let workflow_engine_params_json: any = '';
    ...
    // workflow_tag
    let tags_json: { [key: string]: string } = {};
    for (let i = 0; i < tags.length; i++) {
      const temp_tags_error: string[] = Array.from<string, string>({ length: tags.length }, () => '')
      if (tags[i].key === '') {
    ...
    const tags_json_string = JSON.stringify(tags_json);
    if (tags.length > 0) {
      // console.log("tags", tags_json);
      formData.append('tags', tags_json_string);
    }
    // workflow_tag end
        ...
        <form>
          ...
          <div className="mb-6">
            ...
            <input
              type="string"
              id="workflow_url"
              className={`bg-gray-100 ${workflow_url_error === '' ? '' : 'border border-red-600'
          ...
          <div className="mb-6">
            ...
            <CodeEditor
              ...
              className={`${workflow_params_error === '' ? '' : 'border border-red-600'
          ...
          <div className="mb-6">
            ...
            >
              ...
              <Files
                ...
                onChange={(e: any) => handleAttachmentChange(e)}
                multiple
              ...
              <>
                ...
                >
                  ...
                  <CodeEditor
                    ...
                    className={`${workflow_engine_params_error === ''
                        ? ''
                ...
                <div
                  className={`mb-6 text-gray-900 ${showAdvance ? 'opacity-100' : 'opacity-0'
                    }`}
                  style={{ transition: 'all 0.5s' }}
                >
                  <label
          ...
          <button
            type="submit"
            className={`text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full md:w-36 px-5 py-2.5 text-center flex justify-center ${loading ? 'cursor-wait bg-color2' : 'cursor-pointer  bg-color3'
              }`}
            disabled={loading}
            onClick={async (e) => {
              setLoading(true);
        ...
        <Modal>
          <div className="bg-white rounded-lg p-10 shadow-xl font-open h-fit drop-shadow-2xl">
            <h1 className="mb-8">
              ...
              <div className="text-sm mt-2 ">Please note the run id.</div>
            </h1>
