...
const Content = ({ content }) => {
  const renderBlock = (block) => {
    return block.text.map((segment) => {
      return (
        ...
        >
          {segment.link ? (
            <a key={segment.content} href={segment.link} className={`text-elixirblue hover:underline `}>
              {segment.content}
            </a>
          ) : (
  ...
  return content.map((block) => {
    if (block) {
      if (block.type == "paragraph" || block.type == "heading_1" || block.type == "heading_2" || block.type == "heading_3" || block.type == "callout" || block.type == "quote" || block.type == "code") {
        let textStyle = "";
        if (block.type == "heading_1") {
          ...
          textStyle = "p-2 hover:bg-gray-100  hover:shadow-md shadow-lg border rounded-md text-center";
        ...
        return (
          <Zoom>
            <div className={`leading-relaxed my-3 tracking-wide  ${textStyle}`} key={block.id}>
              {/* dark:text-gray-200 */}
              {renderBlock(block)}
            </div>
          </Zoom>
        ...
        return (
          <Zoom>
            <ul className={`leading-relaxed my-3 tracking-wide list-disc`} key={block.id}>
              {/* dark:text-gray-200  */}
