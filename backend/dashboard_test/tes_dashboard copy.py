...
<head>
    ...
    <style>
        ...
        .wf-section { margin-bottom: 40px; border: 1px solid #eee; padding: 20px; border-radius: 8px; }
        .utility-section { margin-bottom: 40px; border: 1px solid #eee; padding: 20px; border-radius: 8px; }
        .batch-section { margin-bottom: 40px; border: 1px solid #eee; padding: 20px; border-radius: 8px; }
    </style>
</head>
<body>
<div class="container">
    ...
    <div class="batch-section">
      ...
      <h3>Batch/Federated Workflow Runs</h3>
      <table>
        <tr><th>Mode</th><th>TES Instance</th><th>Status</th><th>Run ID</th><th>Log</th></tr>
...
def batch_snakemake():
    ...
    if batch_mode == 'all':
        for inst in TES_INSTANCES:
            ...
            with open(log_file, 'w') as logf:
                subprocess.Popen(cmd, stdout=logf, stderr=logf, env=env)
            batch_runs.append({'mode': 'all', 'tes_name': inst['name'], 'status': 'SUBMITTED', 'run_id': run_id, 'log_file': log_file})
        ...
        with open(log_file, 'w') as logf:
            subprocess.Popen(cmd, stdout=logf, stderr=logf, env=env)
        batch_runs.append({'mode': 'gateway', 'tes_name': 'TES Gateway', 'status': 'SUBMITTED', 'run_id': run_id, 'log_file': log_file})
        save_batch_runs(batch_runs)  # <-- ADD THIS LINE
        run_ids.append(run_id)
